...
//random exponent with 

ret_val = ioctl(dd, IOCTL_READ_TRNG_FIFO, &trng_val);
		if(ret_val == 0) {
			MontExp_4096_test.e[0] = trng_val;
		} else{
			printf("Error\n");
		}

	MontExp_4096_test.e[0] &= 0X7FFFFFFF; 


	for(i=1; i<MontExp_4096_test.prec/32; i++){
		ret_val = ioctl(dd, IOCTL_READ_TRNG_FIFO, &trng_val);
		if(ret_val == 0) {
			MontExp_4096_test.e[i] = trng_val;
		} else{
			printf("Error\n");
		}
	}
	...
	printf("random exp\n\n");
	FILE *fwrite = fopen("/home/alice/AliceSecretExpVal.txt", "w");
	
	char AliceSecretExpVal_hex [128]= "";
    for(i=0 ; i< MontExp_4096_test.prec/32; i++)
	{
        sprintf(AliceSecretExpVal_hex, "%08x", MontExp_4096_test.e[i]);
        fprintf(fwrite,"%s",AliceSecretExpVal_hex);
    }
	...
	FILE *f_write = fopen("/home/alice/AliceP.txt", "w");
	
	char AliceP_hex [128]= "";
    for(i=0 ; i< MontExp_4096_test.prec/32; i++)
	{
        sprintf(AliceP_hex, "%08x", MontExp_4096_test.c[i]);
        fprintf(f_write,"%s",AliceP_hex);
    }	
