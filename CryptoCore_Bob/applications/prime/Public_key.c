...
ret_val = ioctl(dd, IOCTL_READ_TRNG_FIFO, &trng_val);
	...
	printf("\n\n");
	FILE *fwrite = fopen("/home/bob/BobSecretExpVal.txt", "w");
	
	char BobSecretExpVal_hex [128]= "";
    for(i=0 ; i< MontExp_4096_test.prec/32; i++)
	{
        sprintf(BobSecretExpVal_hex, "%08x", MontExp_4096_test.e[i]);
        fprintf(fwrite,"%s",BobSecretExpVal_hex);
    }
	...
	FILE *f_write = fopen("/home/bob/BobP.txt", "w");
	
	char BobP_hex [128]= "";
    for(i=0 ; i< MontExp_4096_test.prec/32; i++)
	{
        sprintf(BobP_hex, "%08x", MontExp_4096_test.c[i]);
        fprintf(f_write,"%s",BobP_hex);
    }	
